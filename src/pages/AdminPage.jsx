    ...
    <div className="flex flex-col min-h-screen bg-black">
      ...
      <header className="sticky top-0 z-30 bg-black/80 backdrop-blur-md border-b border-[#2f3336]">
        <div className="px-4 py-3">
          ...
          <p className="text-xs text-gray-500">Platform Overview & Management</p>
        </div>
        ...
        <div className="flex border-b border-[#2f3336]">
          ...
          >
            Statistics
            {activeTab === 'stats' && <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-14 h-1 bg-[#c30F45] rounded-full" />}
          </button>
          <button 
          ...
          >
            Users
            {activeTab === 'users' && <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-10 h-1 bg-[#c30F45] rounded-full" />}
          </button>
        </div>
      </header>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-[#16181c] p-4 rounded-2xl border border-[#2f3336]">
                ...
                <p className="text-3xl font-extrabold text-white">{stories.length}</p>
              </div>
              <div className="bg-[#16181c] p-4 rounded-2xl border border-[#2f3336]">
            ...
            <div className="bg-[#16181c] p-4 rounded-2xl border border-[#2f3336] h-80">
              ...
              <ResponsiveContainer width="100%" height="90%">
                <BarChart data={topAuthorsChartData}>
                  ...
                  <YAxis hide />
                  <Tooltip cursor={{fill: 'rgba(255,255,255,0.05)'}} contentStyle={{backgroundColor: '#000', border: '1px solid #2f3336'}} />
            ...
            <div className="bg-[#16181c] p-4 rounded-2xl border border-[#2f3336] h-80">
              ...
              <ResponsiveContainer width="100%" height="90%">
                <PieChart>
                  <Pie data={genreChartData} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                    {genreChartData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                  </Pie>
                  <Tooltip contentStyle={{backgroundColor: '#000', border: '1px solid #2f3336'}} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            {users.map(u => (
              <div key={u.id} className="p-4 bg-[#16181c] rounded-2xl border border-[#2f3336] flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  <img src={u.photoURL} className="w-12 h-12 rounded-full border border-[#2f3336]" alt="" />
